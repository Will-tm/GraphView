...
abstract public class GraphView extends LinearLayout
{
	private class GraphViewContentView extends View
	{
		...
		protected void onDraw(Canvas canvas)
		{
			...
			float border = graphViewStyle.getBorderSize();
			float horstart = 0;
			...
			{
				paint.setColor(i == verlabels.length - 1 ? Color.BLACK : graphViewStyle.getGridColor());
				paint.setStyle(Paint.Style.STROKE);
				float y = ((graphheight / vers) * i) + border;

				if (i < verlabels.length - 1) paint.setPathEffect(new DashPathEffect(new float[] {4,2}, 0));
				Path path = new Path();
				path.moveTo(horstart, y);
				path.lineTo(width, y);
				if (i > 0) canvas.drawPath(path, paint);
	...
	private class VerLabelsView extends View
	{
		...
		{
			super(context);
			setLayoutParams(new LayoutParams(getGraphViewStyle().getVerticalLabelsWidth() == 0 ? 100 : getGraphViewStyle().getVerticalLabelsWidth(),
		...
		protected void onDraw(Canvas canvas)
		{
			...
			if (getGraphViewStyle().getVerticalLabelsWidth() == 0 && getLayoutParams().width != verLabelTextWidth + graphViewStyle.getBorderSize())
			{
				setLayoutParams(new LayoutParams((int) (verLabelTextWidth + graphViewStyle.getBorderSize()), LayoutParams.MATCH_PARENT));
			}
			...
			for (int i = 0; i < verlabels.length; i++)
			{
				float y = ((graphheight / vers) * i) + border + labelTextHeight / 2;
	...
	protected void drawHorizontalLabels(Canvas canvas, float border, float horstart, float height, String[] horlabels, float graphwidth)
	{
		...
		{
			paint.setColor(i == 0 ? Color.BLACK : graphViewStyle.getGridColor());			
			float x = ((graphwidth / hors) * i) + horstart;
			if (i == 0) x = x + 1.0f;
			if (i > 0) paint.setPathEffect(new DashPathEffect(new float[] {4,2}, 0));
			Path path = new Path();
			path.moveTo(x, height - border);
			path.lineTo(x, border);
	...
	protected void drawLegend(Canvas canvas, float height, float width)
	{
		...
		switch (legendAlign)
		{
			...
			case MIDDLE:
				...
				break;
			default:
				lTop = height - graphViewStyle.getBorderSize() - legendHeight - getGraphViewStyle().getLegendMarginBottom();
		}
		...
		for (int i = 0; i < graphSeries.size(); i++)
		{
			...
			canvas.drawRect(new RectF(lLeft + border, lTop + border + (i * (shapeSize + spacing)), lLeft + border + shapeSize, lTop + border
					+ (i * (shapeSize + spacing)) + shapeSize), paint);
			if (graphSeries.get(i).description != null)
			{
				paint.setColor(Color.WHITE);
				paint.setTextAlign(Align.LEFT);
				canvas.drawText(graphSeries.get(i).description, lLeft + border + shapeSize + spacing, lTop + border + shapeSize + (i * (shapeSize + spacing)),
						paint);
			}
		}
	}

	abstract protected void drawSeries(Canvas canvas, GraphViewDataInterface[] values, float graphwidth, float graphheight, float border, double minX,
